...
module swap::interface {
    ...
    public entry fun multi_remove_liquidity_with_value<X, Y>(
        global: &mut Global,
        lp_coin: vector<Coin<LP<X, Y>>>,
        lp_val: u64,
        ctx: &mut TxContext
        ...
        let merged_lp = vector::pop_back(&mut lp_coin);
        pay::join_vec(&mut merged_lp, lp_coin);
        let redeemLp = split(&mut merged_lp, lp_val, ctx);

        // 2. handle remain coins
        if (value(&merged_lp) > 0) {
            transfer::public_transfer(
                merged_lp,
                tx_context::sender(ctx)
            )
        } else {
            destroy_zero(merged_lp)
        };
